...
const CreateOrUpdateContact: React.FC<Props> = ({ item }) => {
  ...
  const handleSubmit = async (values: FormValues, { setSubmitting }: any) => {
    dispatch({ type: "ADD_OR_UPDATE_CONTACT", payload: { values, index: item.index } });
    closeModal();
    setSubmitting(false);
  };
  return (
    ...
    >
      {({ values, handleChange, handleBlur, isSubmitting }: FormikProps<FormValues>) => (
        <Form>
          ...
          <Button disabled={isSubmitting} type="submit" style={{ width: "100%", height: "44px" }}>
            <FormattedMessage id="savedContactId" defaultMessage="Save Contact" />
          </Button>
        </Form>
      )}
    </Formik>
