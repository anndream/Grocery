...
const Tree = React.memo(
  ({
    ...
    const Icon = ({ iconName, style }: { iconName: any; style?: any }) => {
      ...
      return !!TagName ? <TagName style={style} /> : <p>Invalid icon {iconName}</p>;
    };
    ...
    return (
      <Frame depth={depth}>
        <Header open={isOpen} depth={depth} className={depth}>
          ...
          {dropdown === true && (
            <Button onClick={() => setOpen(!isOpen)} variant="text" className="toggleButton">
    ...
    return children.map(subOption => {
      ...
      return (
        <Tree
          ...
          defaultOpen={
            // active === subOption.slug || subOption.children.some(item => item.slug === active)
            false
          }
        >
