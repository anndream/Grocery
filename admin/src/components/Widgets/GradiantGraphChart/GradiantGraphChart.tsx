...
import { Box, BoxTitleWrapper, BoxContent } from "components/Admin/Box/Box";
...
const GradiantGraphChart = ({ series, topRowTitle, bottomRowData, labels, colors }: any) => {
  const options = {
    options: {
      chart: {
        toolbar: {
  ...
  return (
    <Box>
      ...
      <BoxContent style={{ display: "block" }}>
        <Chart options={options.options} series={options.series} width="100%" type="area" />
      </BoxContent>

      <BoxTitleWrapper style={{ justifyContent: "space-between", paddingTop: "0" }}>
        {bottomRowData &&
